...
public class ElderlyProfileService {
    ...
    public ElderlyProfileResponse create(ElderlyProfileRequest request) {
        User user = userRepository.findById(request.getUserId())
            .orElseThrow(() -> new NotFoundException("User không tồn tại: " + request.getUserId()));

        if (elderlyProfileRepository.findByUserIdAndDeletedAtIsNull(request.getUserId()).isPresent()) {
        ...
        ElderlyProfile profile = ElderlyProfile.builder()
            ...
            .emergencyContacts(mapContacts(request.getEmergencyContacts()))
    ...
    public ElderlyProfileResponse update(Long id, ElderlyProfileRequest request) {
        ...
        if (request.getEmergencyContacts() != null) {
            profile.setEmergencyContacts(mapContacts(request.getEmergencyContacts()));
        }
    ...
    private List<EmergencyContact> mapContacts(List<EmergencyContactDto> dtos) {
