...
interface MedicationListState {
    ...
    /** Logs của TẤT CẢ thuốc — dùng cho biểu đồ tuân thủ theo ngày. */
        ...
        try {
            const results = await Promise.allSettled(
                items.map(async (m) => {
                    ...
                    // Gắn medicationId từ item phòng khi backend không trả field này
                    return asListOfMaps(resp.data).map((j) => ({
                        ...parseLogEntry(j),
                        medicationId: String(j.medicationId ?? m.id),
                    }));
                }),
            );
